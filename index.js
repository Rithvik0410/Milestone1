...
app.post('/articles', (req, res) => {
        const {id, link, content } = req.body;
        if (!id|| !link||!content) {
          return res.status(400).json({ message: 'Missing required fields: title, content, and tags' });
        }
        const newArticle = {
          id, 
          link,
...
articles.forEach((doc) => {
    const words = doc.content.split(/\W+/); 
    words.forEach((word) => {
...
app.get('/articles/search', (request, response) => {
    const input = request.query.input?.toLowerCase();
    console.log('User Search:', input);
    if (!input) {
        return response.status(400).send({ error: "Input query parameter is required" });
    }
